...
import { AccountNotFoundError, InvalidIdError } from '../../src/entities/errors';
...
describe('GetAccountUseCase', () => {
  it('returns account data when the account exists', async () => {
    ...
    await seedAccount(gateway, { id: VALID_UUID, owner: 'Alice', balance: 250 });
  ...
  it('throws AccountNotFoundError when the account does not exist', async () => {
    ...
    await expect(
      useCase.execute({ accountId: VALID_UUID }),
  ...
  it('throws InvalidIdError when the id is not a valid UUID', async () => {
    ...
    await expect(
      useCase.execute({ accountId: 'not-a-uuid' }),
  ...
  it('throws InvalidIdError for a numeric id', async () => {
    ...
    await expect(
      useCase.execute({ accountId: '12345' }),
    ).rejects.toThrow(InvalidIdError);
  });

  it('retrieves the correct account when multiple accounts exist', async () => {
    const { useCase, gateway } = buildUseCase();
    await seedAccount(gateway, { id: VALID_UUID, owner: 'Alice', balance: 100 });
    await seedAccount(gateway, { id: ANOTHER_UUID, owner: 'Bob', balance: 200 });
